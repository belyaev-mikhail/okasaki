...
data class SZipper<E>
        private constructor(
                ...
                val size: Int = cursor + right.size) {
...
operator fun<E> SZipper<E>.iterator() = gotoFront().iteratorHere()
...
operator fun<E> SZipper.Companion.invoke(slist: SList<E>) = SZipper(slist)
operator fun<E> SZipper.Companion.invoke(e: E) = SZipper(SList(e))
operator fun<E> SZipper.Companion.invoke(vararg e: E) = SZipper(SList(*e))
...
data class SZipperIterator<E>(var data: SZipper<E>): ListIterator<E> {
    ...
    override fun next() = data.current.apply { data = data.goRight() }
