...
// for some reason, Kotlin does not smartcast values with recursively bounded generic parameters
// and doing it inplace is ugly
@Suppress(Warnings.NOTHING_TO_INLINE, Warnings.UNCHECKED_CAST)
inline fun `fix the smartcasting`(t: Treap<*, *>) = t as Treap<Comparable<Any?>, *>
...
data class Treap<E : Comparable<E>, out P>(
    ...
    // to extend the companion object, you should at least define it
    companion object {}

    override fun equals(other: Any?): Boolean =
            if (other is Treap<*, *>)
                iteratorEquals(iterator(), `fix the smartcasting`(other).iterator())
            else false
